...
The main task of this work is to describe and compare tools, that helps in the process of creating and deploying Linux-based operating system on embedded devices.
There are many publications, that prioritize one build systems or are focused on low-level problems common for all, but there was a lack of treating them as one type of software.
...
Growing costs of employment are also forcing companies to shorten time to market with the use of existing software components, especially free and open source ones. The overwhelming use of Linux in modern embedded devices, instead of traditional bare-metal programming, is shown on the Figure \ref{fig:iot-os}.
...
Despite that every aspect of computing could be described by numbers and mathematical operations, there are plenty of definitions. Unfortunately the naming conventions could be misleading, as it is in the area which I am describing. People understand the word ``Linux'' differently in different contexts, as there is also the never ending debate about the term ``GNU/Linux''. Sometimes buzzwords like ``cloud'' and ``IoT'' are strongly promoted due to commercial issues, which makes common understanding even more unclear. Therefore, I would like to explain how to interpret the key wording.
...
% /is using the Linux kernel? - to avoid i.e. "Windows subsystem for Linux" case)
...
The first issue was very clear in the previous (20th) century - making literature review was just analyzing two kind of printed materials: longer ones which were just scientific books and shorter ones which were peer reviewed articles in technical journals.
...
% notes about release process / frequency?

% TODO: foreach
% + add generalized strucutre figure / diagram
% + logo
% + source code / host os / cross-compilation / target os / output
...
% full package list for ultimate builder !!! (add for the end of this chapter
...
% sudo apt install make gcc g++ libncurses-dev unzip git patch python python-dev rsync bc bzip2 gawk zlib1g-dev bison flex tcl gettext lzop gawk diffstat texinfo build-essential chrpath pkg-config
...
# uname -a
Linux buildroot 4.9.52 #1 Sat Jan 20 12:08:00 UTC 2018 armv6l GNU/Linux
\end{verbatim}
...
buildroot login: root
# uname -a
Linux buildroot 4.9.52-v7 #1 SMP Sat Jan 20 11:09:16 UTC 2018 armv7l GNU/Linux
\end{verbatim}
    \caption{buildroot - raspberrypi2}
\end{figure}
...
buildroot login: root
# uname -a
Linux buildroot 4.13.3 #1 SMP Sat Jan 20 15:30:25 UTC 2018 armv7l GNU/Linux
\end{verbatim}
    \caption{buildroot - wandboard}
\end{figure}
...
\begin{lstlisting}
sudo apt install make gcc g++ libncurses-dev unzip git gawk zlib1g-dev
...
--------------------------------------------------
root@LEDE:/# uname -a
Linux LEDE 4.4.92 #0 Tue Oct 17 17:46:20 2017 armv6l GNU/Linux
\end{verbatim}
...
\begin{lstlisting}
sudo apt install make gcc g++ libncurses-dev unzip rpm bison tcl zlib1g-dev
...
% yocto project manifest repository
...
Linux raspberrypi 4.4.50 #1 Sat Jan 20 21:18:12 UTC 2018 armv6l GNU/Linux
\end{verbatim}
...
\begin{figure}
    \centering
\begin{verbatim}
Poky (Yocto Project Reference Distro) 2.2.3 raspberrypi2 /dev/ttyAMA0
raspberrypi2 login: root
root@raspberrypi2:~# uname -a
Linux raspberrypi2 4.4.50 #1 SMP Sat Jan 20 20:07:35 UTC 2018 armv7l GNU/Linux
\end{verbatim}
    \caption{yoctoproject - raspberrypi2}
\end{figure}
...
% cloud is very good idea i.e. because of network connection speed, reiliability and stability
...
\begin{table}
  \begin{tabular}{| p{2.5cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} |}
    ...
    \hline
  \end{tabular}
  \caption{dedicated server / buildroot}
\end{table}
...
\begin{table}
  \begin{tabular}{| p{2.5cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} |}
    ...
    \hline
  \end{tabular}
  \caption{dedicated server / openwrt}
\end{table}
...
\begin{table}
  \begin{tabular}{| p{2.5cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} |}
    ...
    \hline
  \end{tabular}
  \caption{dedicated server / ptxdist}
\end{table}
...
\begin{table}
  \begin{tabular}{| p{2.5cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} |}
    ...
    \hline
  \end{tabular}
  \caption{dedicated server / yoctoproject}
\end{table}
...
\begin{table}
  \begin{tabular}{| p{2.5cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} |}
    ...
    \hline
  \end{tabular}
  \caption{Comparison of VM size and build time (CPU)}
\end{table}
...
\begin{table}
  \begin{tabular}{| p{2.5cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} | p{3cm} |}
    ...
    \hline
  \end{tabular}
  \caption{Comparison of VM size and build time (RAM)}
\end{table}
...
\chapter{Node.js IoT application}
...
\section{Deploying application on embedded device}
...
\section{Possible enchantments}

\subsection*{Including rpm setup}
The description was simplyfied to focus only on debia based distributions , only because of my experience. it should be enchanced.


\subsection*{Regular yearly updates}
Source code development and evolution of standards is so rapid nowadays, that only plain ANSI C, without any libraries, could be considered truly as portable and stable in longer period.
The same issue is happening here, in the area of embedded Linux distributions development tools.
The only way to make this publication usable for others, is to update it constantly and yearly or even bi-yearly circles.

\subsection*{Continious integration}
According to previous statement, it will be reasonable to include modern develpment practicies, like continious integration, to make things easier to maintain. Each of build scripts, that are attached here, should be executed on CI server whenever new stable version of particular tool is released, including new versions of host OS.
...
\\
W rozdziale 4 przedstawione są wyniki eksperymentalnego wykonywania procesu budowy. Ze względu na wielką ilość obliczeń do wykonania oraz konieczność zapewnienia niezawodnego łącza o stałej prędkości, wszystkie procesy są wykonywane na specjalnie wynajętym do tego celu dedykowanym serwerze z procesorami Intel Xeon. Zbierane są metryki takie jak czas i całkowita wielkość folderu budowy, rozmiar systemu operacyjnego i czas od włączenia urządzenia do pojawienia się zachęty logowania. Eksperymenty podzielone są na dwie części: w pierwszej dla stałej mocy obliczeniowej wykonywane są procesy budowania dla różnych systemów i urządzeń wbudowanych, natomiast w drugiej pokazana jest zależność między szybkością maszyny a czasem trwania budowania.
\\
W rozdziale 5 omówiona jest aplikacja przygotowana przez autora wcześniej w ramach projektu inżynierskiego oraz metoda jej wdrożenia na urządzenie wbudowane. Ponieważ głównym wymaganiem jest zainstalowanie środowiska uruchomieniowego Node.js, w związku z tym przedstawiono sposoby na dodanie go poprzez każdy system budowania. Tak przygotowane urządzenie może być wykorzystywane jako brama Internetu Rzeczy.
\\
W rozdziale 6 ukazany jest sposób tworzenia kontenerów przy pomocy każdego omówionego systemu budowania. Kontenery, opisywane jako lekkie maszyny wirtualne, są obecnie coraz częściej wykorzystywane w zastosowaniach produkcyjnych, szczególnie chmurowych. Przygotowany obraz systemu operacyjnego jest kompatybilny z popularnymi narzędziami orkiestracji, w tym przede wszystkim z Kubernetesem.
\\
W rozdziale 7 wysnute są wnioski i rekomendacje na dalszy rozwój tej publikacji oraz samych narzędzi budowania dystrybucji Linuxa. Przede wszystkim zawartość musi być aktualizowana przynajmniej raz do roku, ze względu na częste zmiany, jakie zachodzą w wolnym oprogramowaniu. Finalnie wyrażony jest niepokój, że mimo posiadania szerokiej gamy uniwersalnych narzędzi, wciąż obsługują one jedynie jądra Linuxa. Jak w każdym obszarze, brak konkurencji może w dłuższej perspektywie doprowadzić do spadku jakości. Autor wyraża nadzieje, że wykorzystując te same narzędzia będzie można w przyszłości zbudować systemy oparte o jądra MINIX, BSD, Darvin oraz HURD.
